  ...
  const alertList = useMemo(() => {
    ...
    if (nextAlerts.length === 0) {
      nextAlerts.push({
        ...
        message: 'Cuando el backend detecte presupuestos en riesgo o tendencias relevantes, apareceran aqui.',
        date: 'Sistema',
        icon: CheckCircle2,
        ...
        <header className="flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-gray-400 font-medium">Lectura en tiempo real de presupuestos y resultados ML</p>
            </div>
          </div>
          <button onClick={() => setShowSettings(true)} className="p-4 glass rounded-2xl hover:bg-white/10 transition-all group">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-white/[0.02] backdrop-blur-3xl border border-white/10 rounded-[3rem] p-10 shadow-2xl">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setSelectedAlert(null)} className="p-2 hover:bg-white/5 rounded-full text-gray-500 transition-colors">
                  <X size={24} />
                </button>
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-white/[0.02] backdrop-blur-3xl border border-white/10 rounded-[3rem] p-10 shadow-2xl">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setShowSettings(false)} className="p-2 hover:bg-white/5 rounded-full text-gray-500 transition-colors">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    return (
                      <div key={setting.key} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                        <span className="text-sm font-bold text-gray-300">{setting.label}</span>
                        <div onClick={() => updateAlertSettings({ [setting.key]: !isActive })} className={`w-12 h-6 rounded-full relative transition-colors cursor-pointer ${isActive ? 'bg-purple-600' : 'bg-gray-700'}`}>
                ...
                <div className="p-5 bg-purple-600/10 border border-purple-500/20 rounded-2xl text-sm text-gray-300 flex items-start gap-3">
                  ...
                  <span>Estas preferencias se guardan localmente en el navegador para no bloquearte mientras defines una tabla dedicada en backend.</span>
                </div>
              </div>
            </motion.div>
          </div>
